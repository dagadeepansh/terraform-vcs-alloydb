 ...
 */
module "alloydb_primary_bronze" {
  source = "../../../../terraform-gcp-alloydb"
  project_id     = "cloudlake-dev-1"
  region_primary = "us-central1"
  cluster_id     = "primary-bronze-cluster-id"
  cluster_name   = "primary-cluster-bronze" 
  ...
  backup_labels                  = {test = "alloydb-cluster-bronze"} # Use distinct labels

  continuous_backup_recovery_window_days = 10

  primary_instance = {
    instance_id  = "primary-instance-bronze-us-central1"
    display_name = "Primary Instance bronze (us-central1) "
